...
# We will use window-size arguments to keep it light for your MacBook.
HEADLESS_MODE = False 
...
def fetch_with_playwright(url, domain):
    ...
    try:
        with sync_playwright() as p:
            ...
            # CONFIGURATION: Headless=False (Visible) to fix Komplett
            # Window size 1280x720 to save resources
            ...
            try:
                # WAIT STRATEGY: 'networkidle' for NetOnNet, 'domcontentloaded' for others (faster)
...
def fetch_page_content(url):
    ...
    # Mandatory Playwright list (Komplett needs this to be visible to work)
...
# Now detects AMD cards and has a wider vision range
def extract_regex_raw(soup, domain):
    ...
    for match in matches:
        ...
        # INCREASED CONTEXT: 300 chars to find the name in messy outlets
        start = max(0, match.start() - 300)
        end = match.start()
        context = text[start:end]
        
        # KEYWORDS: Added AMD support (RX, XT, Radeon) and generic "Grafikkort"
        keywords = ["RTX", "GeForce", "Radeon", "RX", "XT", "Grafikkort", "Gaming"]
        if any(k in context for k in keywords):
            parts = re.split(r'[|•\n]', context)
            potential_name = parts[-1].strip()
            # If name is too short, grab the chunk before it too
...
def parse_products(soup, domain, url, api_data=[]):
    ...
    if "netonnet.se" in domain:
        ...
        # Fallback to Regex if scripts failed
